...
export class AudioService implements vscode.Disposable {
    ...
    private async resolveAudioBackend(configured: SoundBackendSetting): Promise<AudioBackendState> {
    ...
    private async tryEnableNativeHelper(configured: SoundBackendSetting): Promise<AudioBackendState | undefined> {
        this.disposeNativeHelper();
        this.nativeHelper = new NativeHelperBackend(this.context);
